...
@pytest.mark.asyncio
async def test_start_stop_toggles_run_controls_state() -> None:
    app = K6TestApp()
...
@pytest.mark.asyncio
async def test_invalid_vu_shows_warning_and_does_not_call_scale() -> None:
    app = K6TestApp()
    fake_controller = FakeRunController()
    notifications: list[tuple[str, str]] = []
...
@pytest.mark.asyncio
async def test_external_mode_warning_and_controls_disabled_when_running() -> None:
    app = K6TestApp()
    fake_controller = FakeRunController()
    ...
    async with app.run_test() as pilot:
        ...
        assert "Stop and scale actions from UI are unavailable." in external_mode_warning.renderable
        assert run_btn.disabled is True
        assert stop_btn.disabled is True
        assert apply_vu_btn.disabled is True
